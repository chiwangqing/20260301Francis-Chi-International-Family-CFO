...
const ScrollingAlbum: React.FC = () => {
  const storyNames = [
    'story-1', 'story-2', 'story-3', 'story-4', 'story-8', 
    'story-10', 'story-11', 'story-12', 'story-15', 'story-17', 
    'story-19', 'story-20', 'story-21', 'story-22', 'story-23', 
    'story-24', 'story-25'
  ];
  
  const images = storyNames.map(name => ASSETS.STORY_IMAGE(name));
        ...
        >
          {images.concat(images).map((src, index) => (
            ...
            >
              <img
                src={src}
                alt={`Story ${index % 25 + 1}`}
                className="w-full h-full object-cover"
                referrerPolicy="no-referrer"
                loading="lazy"
                onError={(e) => {
                  const target = e.target as HTMLImageElement;
                  target.src = `https://placehold.co/800x600/1a1a1a/D4AF37?text=Story+${index % 25 + 1}+Unavailable`;
                }}
              />
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity bg-black/20">
                 <span className="text-gold/40 text-sm font-serif">Brand Story {index % 25 + 1}</span>
              </div>
            </div>
          ))}
        </motion.div>
